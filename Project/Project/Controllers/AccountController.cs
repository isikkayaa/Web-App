...

    {
...

        {
            ...
            if (ModelState.IsValid)
            {
                ...
                User user = _databaseContext.UserData.SingleOrDefault(x => x.Username.ToLower() == model.Username.ToLower() && x.Password == hashedPassword);   /*kontrol yapmak icin ve kayıt varsa bize döncek bi tane*/
        ...
        public IActionResult Register(RegisterViewModel model) /*//Register sayfamız icin*/
        {
            if (ModelState.IsValid)
            {
                ...
                _databaseContext.UserData.Add(user);  /*yeni nesnemi user tablosuna ekledim*/
        ...
        private void ProfileInfoLoader()
        {
            ...
            User user = _databaseContext.UserData.SingleOrDefault(x => x.Id == userid);
        ...
        public IActionResult ProfileChangeImage([Required]IFormFile file)
        {
            if (ModelState.IsValid)
            {
                ...
                stream.Close();
                stream.Dispose();         /*yok etmek icin bu nesneyi*/
